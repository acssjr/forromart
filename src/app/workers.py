            ...
            for course_id, course_data in self.selection.items():
                ...
                for module_index, module in enumerate(course_data.get("modules", []), start=1):
                    ...
                    for lesson_index, lesson in enumerate(module.get("lessons", []), start=1):
                        ...
                        org_mode = getattr(self.settings, "folder_organization_mode", "legacy")
                        if org_mode == "flat":
                            lesson_path = module_path
                        elif org_mode == "folders_descriptive":
                            lesson_title_full = f"{lesson_order:02d}. {lesson_title}"
                        ...
                        try:
                            ...
                            if lesson_details.description:
                                if getattr(self.settings, "skip_description_download", False):
                                    ...
                                    if org_mode == "flat":
                                        ...
                                        description_name = f"Descrição.{desc_suffix}"
                                    description_name = truncate_filename_preserve_ext(description_name, getattr(self.settings, 'max_file_name_length', 30))
                                    description_path = lesson_path / description_name
                            ...
                            if getattr(self.settings, "skip_video_download", False):
                                ...
                                for video_index, video in enumerate(lesson_details.videos, start=1):
                                    ...
                                    has_multiple_videos = len(lesson_details.videos) > 1
                            ...
                            if getattr(self.settings, "skip_attachment_download", False):
                                ...
                                for attachment_index, attachment in enumerate(lesson_details.attachments, start=1):
                                    ...
                                    if org_mode == "flat":
                                        ...
                                        full_attachment_name = f"{attachment_order}. {sanitized_filename}"
                            ...
                            if lesson_details.auxiliary_urls:
                                if getattr(self.settings, "skip_auxiliary_urls_download", False):
                                    ...
                                    if org_mode == "flat":
                                        ...
                                        aux_name = "Links Extras.txt"
                                    aux_name = truncate_filename_preserve_ext(aux_name, getattr(self.settings, 'max_file_name_length', 30))
                                    aux_path = lesson_path / aux_name
