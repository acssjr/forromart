...
class SettingsView(QWidget):
    ...
    def _setup_ui(self) -> tuple[QGroupBox, QGroupBox, QGroupBox]:
        ...
        self.try_keep_original_video_name_check = QCheckBox(
            "Tentar manter nome original dos vídeos (nao recomendado)"
        )
        ...
        self.folder_org_combo.addItem("Misto (Subpastas + Videos renomeados)", "folders_descriptive")

        self._form_layout.addRow("Modo de Organizacao de Pastas:", self.folder_org_combo)
        self._form_layout.addRow(self.skip_attachment_download_check)
        self._form_layout.addRow("Baixar apenas anexos com as extensoes (uma por linha):", self.allowed_extensions_edit)
        ...
        self.skip_existing_files_check = QCheckBox("Pular arquivos ja baixados (verifica se o arquivo existe no disco)")
        self._paid_form_layout.addRow(self.skip_existing_files_check)
    ...
    def load_settings(self) -> None:
        ...
        self.skip_existing_files_check.setChecked(getattr(settings, "skip_existing_files", False))

        folder_org_index = self.folder_org_combo.findData(getattr(settings, "folder_organization_mode", "legacy"))
        if folder_org_index != -1:
            self.folder_org_combo.setCurrentIndex(folder_org_index)
    ...
    def save_settings(self) -> None:
        ...
        updated_settings = AppSettings(
            ...
            skip_existing_files=self.skip_existing_files_check.isChecked(),
            pause_on_partial_count=self.pause_on_partial_spin.value(),
            pause_on_error_count=self.pause_on_error_spin.value(),
            folder_organization_mode=self.folder_org_combo.currentData(),
        )
